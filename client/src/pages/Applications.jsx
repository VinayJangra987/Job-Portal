      ...
      <div className="container px-4 min-h-[65vh] 2xl:px-20 mx-auto my-10">
        ...
        <div className="flex gap-2 mb-6 mt-3">
          {isEdit || (userData && userData.resume === "") ? (
            <>
              <label className="flex items-center" htmlFor="reumeUpload">
                <p className="bg-blue-100 text-blue-600 px-4 py-2 rounded mr-2">{resume ? resume.name : "Select Resume"}</p>
                <input id="reumeUpload" onChange={(e) => setResume(e.target.files[0])} accept="application/pdf" type="file" hidden />
                <img src={assets.profile_upload_icon} alt="" />
              </label>
              <button onClick={updateResume} className="bg-green-100 border border-gray-400 rounded-lg px-4 py-2">Save</button>
            </>
          ) : userData ? (
